...
object HiveWindowFunTest2 {
  def main(args: Array[String]): Unit = {
    ...
    //todo  模拟hive 测试二
    //     主要是分析函数：聚合类 + 偏移量类lag lead ntile
    //1、查询在2017年4月份购买过的顾客及总人数
    //    hiveQuery("select *,count(name)over()as total from business where substr(orderdate,1,7)='2017-04'")

//    hiveQuery("select *,sum(case when substr(orderdate,1,7)='2017-04' then 1 else 0 end)over() total from business")
    ...
    //前 第n行
//    hiveQuery("select lag(orderdate,2)over(partition by name order by orderdate) as last_date from business where name='mart'")


    // 5、查询前20%时间的订单信息
    //ntile(n) 把有序分区中的行分发到指定数据的组中，各个组有编号，编号从1开始，对于每一行，ntile返回此行所属的组的编号
//    hiveQuery("select * from (select *,ntile(5)over(order by orderdate) as sortgroup_num from business) t where t.sortgroup_num=1")
